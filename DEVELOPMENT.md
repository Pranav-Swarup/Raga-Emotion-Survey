...
Technical documentation for running, configuring, and deploying this survey. If you're here for the research itself, see `README.md` instead.
...
- **Listen-gated ratings.** On the familiarisation clip and on every music clip, the rating controls stay locked (with a "please finish listening" message) and the Continue/Next button stays invisible until 85% of the audio has played (`LISTEN_THRESHOLD` in `survey-config.js`). For clips, Next also needs all nine GEMS-9 items answered — the free-text word is optional. Only enforced on the first listen; replaying afterwards never re-locks anything.
...
- **Pre-clip notice.** A short screen with a visible countdown (`NOTICE_SECONDS` in `app.js`, default 15s) sits before the first rated clip and between clips, so one clip's mood doesn't bleed into the rating of the next. The first occurrence shows full listening instructions; later ones show just a one-line justification. The next clip's audio preloads during this pause and while the current one is being rated.
- **Back/forward arrows.** Faint arrows pinned to the bottom corners let a respondent step back to a previous screen (to change an answer) and forward again to where they left off. They grey out completely at the two ends of the flow — you can't skip ahead of your furthest answered step, only revisit ones you've already done. The notice screen itself isn't a navigable step.
...
- **Debrief screen.** "Know more about this survey" on the Done screen reveals the hypothesis, method, a melakarta/janya-raga primer, citations, and performer credits — including which raga the respondent heard in which order, generated from their actual randomised `clipOrder`.

## Swap in real clips
Edit the `CLIPS` array in `survey-config.js`: each needs `id`, `label`, `src` (hosted audio URL), `melakarta` / `raga` (stored with each response, never shown to the respondent — the study is a blind comparison), `traditionalEmotion` (shown only on the post-survey debrief screen), and `sourceUrl` (credited on the debrief screen). Replace `FAMILIARISATION_SRC` / `FAMILIARISATION_RAGA` / `FAMILIARISATION_MELAKARTA` with your warm-up clip's details. A Supabase storage bucket is a convenient place to host the audio in the same free project.
...
**Existing project** (already has a `submissions` table from an earlier version of this app): run `migration.sql` instead of `schema.sql` — it only adds new columns and never touches existing rows or drops anything.
...
**Privacy note:** `roll_number` and `name` are only ever filled in when a respondent says they're part of the Music Workshop course — those specific rows are identifiable, unlike the rest of the (anonymous) table.
...
## Data shape
One row per clip per respondent: `respondent_id, clip_id, melakarta, raga, presentation_order, wonder, transcendence, tenderness, nostalgia, peacefulness, power, joyful_activation, tension, sadness, free_text, time_spent_ms, age, training, years_of_training, listening_frequency, familiar, course_student, roll_number, name, created_at`. Clip order is randomised per respondent (familiarisation is always first and excluded from randomisation) and recoverable via `presentation_order`. Demographics are collected once per respondent and repeated on each of their clip rows.

## Notes / easy adjustments
- GEMS-9 is the only instrument — no valence/arousal, no second scale.
- No forced-listen gate on replay — clips can be replayed freely; what's gated on 85% playback is the *first* appearance of the rating controls and the Next/Continue button, per `LISTEN_THRESHOLD` in `survey-config.js`.
